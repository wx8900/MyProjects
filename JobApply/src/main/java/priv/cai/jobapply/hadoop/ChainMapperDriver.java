...
public class ChainMapperDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		FileInputFormat.addInputPath(job, new Path(otherArgs[0]));  // Constants.BIG_DATA_INPUT_PATH;
		FileOutputFormat.setOutputPath(job, new Path(otherArgs[2]));// Constants.BIG_DATA_OUTPUT_PATH;
		
		// Add the user files to the DistributedCache
		FileStatus[] userFiles = FileSystem.get(conf).listStatus(new Path(otherArgs[1]));// Constants.BIG_DATA_CACHE_PATH
